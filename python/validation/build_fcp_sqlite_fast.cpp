...
int main(){
 ...
 for(size_t i=0;i<cap;++i)if(keys[i]){
  ...
  if(sqlite3_step(st)!=SQLITE_DONE)die_sql(db,"insert"); sqlite3_reset(st); n++; sum+=a.purchase; 
 }
